...
export function LiveDataProvider({ children }: { children: ReactNode }) {
  ...
  const getWebSocketUrl = useCallback((): string => {
    ...
    let configuredUrl = process.env.NEXT_PUBLIC_WS_URL?.trim();
    ...
    if (configuredUrl) {
      ...
      // Upgrade to wss:// if current origin is secure HTTPS (Mixed Content prevention)
      if (isHttps && configuredUrl.startsWith('ws://')) {
        configuredUrl = configuredUrl.replace(/^ws:\/\//, 'wss://');
      }

      // If URL was baked-in as localhost, but user is accessing via public tunnel/domain, route through current host
    ...
    // 1. If accessed over remote domain / tunnel (e.g. Cloudflare tunnel, Vercel proxy):
    if (!isLocal) {
      return `${proto}//${window.location.host}/ws/live`;
    }

    // 2. If accessed via local gateway (:3000):
    if (window.location.port === '3000') {
      return `${proto}//${window.location.host}/ws/live`;
    }

    // 3. If accessed via direct Next.js frontend dev port (:3001), connect directly to backend :8000:
