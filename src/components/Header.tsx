...
import { Cpu, FileText, Sun, CloudRain, Moon, ShieldCheck } from 'lucide-react';
  ...
  return (
    <header className="bg-[#0B0F17] border-b border-slate-800/80 px-4 lg:px-6 py-3">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-3">
        ...
        {/* Left: Refined Brand Logo & Subtitle */}
        <div className="flex items-center space-x-3 w-full md:w-auto justify-between md:justify-start">
          ...
          <div className="flex items-center space-x-2 md:hidden">
            <span className={`px-2.5 py-1 rounded-full text-xs font-sans border flex items-center gap-1.5 ${
              ...
              <span>{isLiveBackend ? 'Live Modbus TCP/MQTT' : 'Synthetic Engine'}</span>
            </span>
          </div>
        </div>
        ...
        {/* Right: Segmented Control & Ghost Buttons */}
        <div className="flex items-center space-x-3 w-full md:w-auto justify-end">
          ...
          <div className="bg-slate-900/90 p-1 rounded-lg border border-slate-800 flex items-center space-x-1">
            <button
              ...
              className={`px-3 py-1 rounded text-xs font-sans transition-all flex items-center gap-1.5 ${
                scenario === 'SUNNY_NOON'
          ...
          <div className="flex items-center space-x-2">
            <button
              onClick={onOpenArch}
              className="px-3 py-1.5 rounded-lg bg-slate-800/60 hover:bg-slate-800 text-slate-300 border border-slate-700/60 text-xs font-sans transition-all flex items-center gap-1.5"
            >
